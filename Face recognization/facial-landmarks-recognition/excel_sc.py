...
if not os.path.exists("Teja_screenshots(4)"):
    os.makedirs("Teja_screenshots(4)")
...
try:
    while True:
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('teja_attendance_with_screenshots(4).xlsx', index=False)
            print("DataFrame saved to 'teja_attendance_with_screenshots(4).xlsx'")
    ...
    if not df.empty:
        df.to_excel('teja_attendance_with_screenshots(4).xlsx', index=False)
        print("Final attendance and screenshots saved to 'teja_attendance_with_screenshots(4).xlsx'.")
