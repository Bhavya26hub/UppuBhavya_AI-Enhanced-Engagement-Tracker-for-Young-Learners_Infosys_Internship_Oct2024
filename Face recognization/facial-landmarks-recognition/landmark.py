...
if not os.path.exists("Teja_screenshots(6)"):
    os.makedirs("Teja_screenshots(6)")
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"Teja_screenshots(6)/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                if attentive:
        ...
        if (datetime.now() - last_save_time) > timedelta(seconds=30):
            df.to_excel('Teja_attendance_with_attention(6).xlsx', index=False)
            print("Data saved to 'Teja_attendance_with_attention(6).xlsx'")
    ...
    if not df.empty:
        df.to_excel('Teja_attendance_with_attention(6).xlsx', index=False)
        print("Final data saved to 'Teja_attendance_with_attention(6).xlsx'")
